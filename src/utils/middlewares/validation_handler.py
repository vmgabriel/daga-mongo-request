...
"""Module for Validation Handler"""

import datetime, functools, jwt
...
def get_jwt(cookie):
    ...
    return delete_first_four_in_token(cookie) + '.' + delete_fisrt_and_last(
        cookie
    )


def verify_expiration(data):
    """Verify the Expiration of data"""
    def seconds_counts_to_date(date):
        """Secound Count of date"""
        return datetime.datetime.fromtimestamp(date)
    return seconds_counts_to_date(data['exp']) > datetime.datetime.now()


def verify_ip(data, _ip):
    """Verify IP"""
    return data == _ip
...
def verify_permisssion(data, module, permission):
    ...
    def do_all_in_array(_fn):
        """Do all in Array"""
        return lambda arr: [_fn(arr[0])] if (len(arr) == 1) \
            else [_fn(arr[0])] + do_all_in_array(_fn)(arr[1:])

    def get_two_firsts_letters(word):
        """Get two First Letters"""
        return compose(
            ''.join,
...
def verify_cookie(cookies, _ip, module, permission):
    """Verify the Cookie"""
